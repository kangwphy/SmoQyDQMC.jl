...
function _allocate_symmetric_exact_propagators(fpi::FermionPathIntegral{T,E}) where {T,E}
    ...
    for l in 1:Lτ
        ...
        expmΔτV_l = ones(T, N)
        B_l = SymExactPropagator{T, E}(expmΔτV_l, expmΔτKo2_l, exppΔτKo2_l)
        push!(B, B_l)
    end
...
function _allocate_asymmetric_exact_propagators(fpi::FermionPathIntegral{T,E}) where {T,E}
    ...
    for l in 1:Lτ
        ...
        expmΔτV_l = ones(T, N)
        B_l = AsymExactPropagator{T, E}(expmΔτV_l, expmΔτK_l, exppΔτK_l)
        push!(B, B_l)
    end
...
function _allocate_symmetric_checkerboard_propagators(fpi::FermionPathIntegral{T,E}) where {T,E}
    ...
    for l in 1:Lτ
        ...
        expmΔτV_l = ones(T, N)
        B_l = SymChkbrdPropagator{T, E}(expmΔτV_l, expmΔτKo2_l)
        push!(B, B_l)
    end
...
function _allocate_asymmetric_checkerboard_propagators(fpi::FermionPathIntegral{T,E}) where {T,E}
    ...
    for l in 1:Lτ
        ...
        expmΔτV_l = ones(T, N)
        B_l = AsymChkbrdPropagator{T, E}(expmΔτV_l, expmΔτK_l)
        push!(B, B_l)
    end
